// src/pages/GerenciarSalas.jsx 
// (Anteriormente em src/features/salas/GerenciarSalas.jsx)

import React, { useState, useEffect } from 'react';
import { Row, Col, Button, Alert, Modal, Spinner, Card } from 'react-bootstrap';
// Caminhos de importação atualizados:
import { getSalas, deleteSala, addSala, updateSala } from '../services/salaService'; // Subiu um nível e entrou em services
import SalaForm from '../features/salas/SalaForm'; // Subiu um nível e entrou em features/salas
import { FaEdit, FaTrash, FaPlus, FaTv, FaPlusSquare } from 'react-icons/fa';

// NOME DA FUNÇÃO (permanece o mesmo, GerenciarSalas)
function GerenciarSalas() {
  ...
  const handleAbrirModalDelecao = (sala) => { /* ...código da função ... */ 
    setSalaParaDeletar(sala);
    setShowDeleteModal(true);
  };
  const handleFecharModalDelecao = () => { /* ...código da função ... */ 
    setSalaParaDeletar(null);
    setShowDeleteModal(false);
  };
  const handleConfirmarDelecao = () => { /* ...código da função ... */ 
    if (salaParaDeletar) {
      setLoading(true);
      try {
        deleteSala(salaParaDeletar.id);
  ...
  const handleAbrirModalCadastroSala = () => { /* ...código da função ... */ 
    setSalaEmEdicao(null);
    setIsModalEditMode(false);
    setShowSalaModal(true);
  };
  const handleAbrirModalEdicaoSala = (sala) => { /* ...código da função ... */ 
    setSalaEmEdicao(sala);
    setIsModalEditMode(true);
    setShowSalaModal(true);
  };
  const handleFecharSalaModal = () => { /* ...código da função ... */ 
    setShowSalaModal(false);
    setSalaEmEdicao(null);
  };
  const handleSalvarSala = async (dataDaSala) => { /* ...código da função ... */ 
    setFormSubmitting(true);
    setError('');
    setMensagemSucesso('');
    try {
      if (isModalEditMode && salaEmEdicao) {
        updateSala(salaEmEdicao.id, dataDaSala);
  ...
  // O restante do componente (JSX e lógica interna) permanece o mesmo
  // ... (copie o restante do return e a lógica do seu GerenciarSalas.jsx aqui) ...
  ...
  return (
    <div>
              ...
              <Card.Body className="d-flex flex-column">
                ...
                <div className="mt-auto d-flex justify-content-end pt-2">
                  <Button 
                    ...
                    size="sm" 
                    className="me-2" 
                    onClick={() => handleAbrirModalEdicaoSala(sala)} 
                    title="Editar Sala"
                  >
                  ...
                  <Button 
                    ...
                    size="sm" 
                    onClick={() => handleAbrirModalDelecao(sala)} 
                    title="Deletar Sala"
                  >
      ...
      <Modal show={showDeleteModal} onHide={handleFecharSalaModal} centered>
        <Modal.Header closeButton><Modal.Title><FaTrash className="me-2 text-danger"/>Confirmar Deleção</Modal.Title></Modal.Header>
        <Modal.Body>
          Você tem certeza que deseja deletar a sala: <strong>{salaParaDeletar?.nome}</strong>?
          <br/>Se esta sala estiver associada a sessões, essas sessões podem ficar inconsistentes.
