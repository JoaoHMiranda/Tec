// src/features/salas/SalaForm.jsx
...
function SalaForm({ initialData, onSubmit, onCancel, isEditMode, submitting }) {
  ...
  const handleFormSubmit = (e) => {
    ...
    if (isNaN(parseInt(sala.capacidade, 10)) || parseInt(sala.capacidade, 10) <= 0) {
      setFormError('A capacidade deve ser um número inteiro positivo.');
      return;
    }
    onSubmit(sala); // Chama a função de submissão passada por props
  };
    ...
    <Form onSubmit={handleFormSubmit} noValidate>
      ...
      <div className="mt-4 d-flex justify-content-end">
        ...
        <AppButton type="submit" variant={isEditMode ? "info" : "primary"} disabled={submitting}>
          {submitting ? (isEditMode ? 'Atualizando...' : 'Salvando...') : (isEditMode ? 'Salvar Alterações' : 'Salvar Sala')}
        </AppButton>
      </div>
    </Form>
  );
